...
class Setting extends React.Component {
    ...
    render() {
        ...
        return (<Form layout="horizontal" className="ant-advanced-search-form">
				{data.map((n, i) => (<div key={n.name}>
						{n.title ? <h2>{n.title}</h2> : ''}
						{((n) => {
            switch (n.type) {
                case 'boolean':
                    return (<FormItem help={n.description}>
                ...
                case 'datetime':
                    return (<FormItem help={n.description} label={n.displayName}>
											{getFieldDecorator(n.name, {
                        initialValue: n.value
