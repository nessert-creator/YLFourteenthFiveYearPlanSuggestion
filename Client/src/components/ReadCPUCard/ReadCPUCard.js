...
class ReadCPUCard extends Component {
    ...
    render() {
        ...
        return (<div style={style}>
				<Button disabled={error} loading={loading} type="primary" onClick={() => {
            ...
            this.props.dispatch({
				...
				<span>
					卡号：
					{form.getFieldDecorator('input')(<Search style={{ width: '140px' }} onSearch={(value) => {
            if (onRead) {
                onRead(value);
            }
        }} onChange={(value) => {
...
ReadCPUCard.defaultProps = {
    text: '读取IC卡信息',
    errorText: '控件不可用，可能未正确安装控件及驱动，或者控件未启用。'
};
ReadCPUCard = connect((state) => {
    return Object.assign({}, state.cloudServer);
})(ReadCPUCard);
