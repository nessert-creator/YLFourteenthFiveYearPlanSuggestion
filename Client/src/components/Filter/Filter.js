...
class Filter extends React.Component {
    constructor() {
        super(...arguments);
        this.state = {
            expand: this.props.advancedFilters &&
    ...
    render() {
        ...
        const advancedSearch = useAdvancedSearch ? (<a style={advancedButtonStyle} onClick={() => {
            this.setState({
                expand: true
            });
        }}>
				{useAdvancedSearchOnly ? null : (<span>
						高级搜索 <Icon type="down"/>
					</span>)}
			</a>) : null;
        const _this = this;
        ...
        const mapField = (n) => (<span key={n.fileldName}>
				{((n) => {
            switch (n.type) {
                case 'boolean':
                    return (<FormItem help={n.description} {...formItemProps}>
                ...
                case 'datetime':
                    return (<FormItem help={n.description} label={n.displayName} {...formItemProps}>
									{getFieldDecorator(n.fileldName, {
                        initialValue: n.value,
                        rules: n.rules
                    })(<DatePicker />)}
                ...
                case 'select':
                    return (<FormItem help={n.description} label={n.displayName} {...formItemProps}>
                        ...
                        rules: n.rules
											...
											{n.selectOptions.map((o, i) => (<Option key={i} value={o.value}>
													{o.name}
												</Option>))}
										</Select>)}
								</FormItem>);
        ...
        return !expand ? (<div>
            ...
            (filters[0].type == undefined || filters[0].type == 'string') ? (<div>
							<Row>
								<Col span={8}>
									<Search enterButton placeholder={filters[0].displayName} onSearch={(value) => {
            var values = {};
            ...
            form.resetFields();
							...
							{useAdvancedSearchOnly ? null : (<a style={advancedButtonStyle} onClick={() => {
