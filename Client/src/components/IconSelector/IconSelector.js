...
export default class IconSelector extends Component {
    ...
    render() {
        ...
        return (<Popover content={<div style={{ overflowY: 'auto', width: '420px', height: '320px' }}>
						<div style={{ borderBottom: '1px', width: '400px', height: '300px' }}>
							<Row>
								{iconClasses.map((name, index) => {
            return (<Col span={2} style={{ marginTop: 2 }} key={index}>
											<Button icon={name} onClick={onIconSelected.bind(this, name)}/>
										</Col>);
        })}
							</Row>
						</div>
					</div>} title={<center>图标列表</center>} placement="bottomLeft" visible={selectorVisible} trigger="click" onVisibleChange={handleVisibleChange}>
