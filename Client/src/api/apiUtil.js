...
function callApi(method, params, options) {
    var result = method.apply(this, method.length == 2
        ? [params, options]
...
export function createApiAuthParam(parm) {
    let token = localStorage.getItem('token');
    let payload = parm.payload ? parm.payload : {};
    let param = Object.assign({}, payload, { input: payload //post参数
     });
    let options = {
