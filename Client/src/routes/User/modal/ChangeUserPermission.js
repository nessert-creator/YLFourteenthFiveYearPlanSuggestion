 ...
 * 描述: 修改用户权限弹窗
 */
function ChangeUserPermission({ dispatch, currentUser, currentEditUserPermission, changeUserPermissionModalState, selectPermission }) {
    function handleCancel() {
        dispatch({
            type: 'user/setState',
            payload: {
                changeUserPermissionModalState: Object.assign({}, changeUserPermissionModalState, { visible: false })
            }
        });
    }
    const loopPermissions = (data) => data.map((item) => {
    ...
    return (<Modal visible={changeUserPermissionModalState.visible} title={'修改权限 - ' + (currentUser ? currentUser.name : '')} onCancel={handleCancel} footer={[
			...
			<Tabs defaultActiveKey="1">
				<TabPane tab="操作权限" key="1">
					<Tree checkable onCheck={onCheck} checkedKeys={selectPermission}>
						{loopPermissions(currentEditUserPermission.permissions ? currentEditUserPermission.permissions : [])}
					</Tree>
				</TabPane>
			</Tabs>
		</Modal>);
