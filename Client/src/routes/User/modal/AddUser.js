...
function AddUser({ dispatch, pagination, form, loading, addUserModalState, roles, selectedOrganizations }) {
    function handleCancel() {
        dispatch({
            type: 'user/setState',
            payload: {
                addUserModalState: Object.assign({}, addUserModalState, { visible: false })
            }
        });
    }
    ...
    function onTabChange(value) {
        dispatch({
            type: 'user/setState',
            payload: { addUserModalState: Object.assign({}, addUserModalState, { activeTabKey: value }) }
        });
    }
    ...
    return (<Modal visible={addUserModalState.visible} width={658} title="添加用户" onCancel={handleCancel} footer={[
            ...
            form.validateFields((err, values) => {
                ...
                if (!err) {
                    dispatch({
                        ...
                        payload: Object.assign({}, data, { pagination })
                    });
                }
            });
        }}>
        ...
        initialValue: true
				...
				<TabPane tab={<div className={styles.mm}>
							<span>角色</span>
							<Badge count={getSelectedRoleCount()} style={{ backgroundColor: '#5D7DC3', marginLeft: 4 }}/>
						</div>} key="2">
					<Form>
						{roles.map((ele, index) => {
...
AddUser = connect((state) => {
    return Object.assign({}, state.user, { loading: state.loading.effects['user/createOrUpdateUser'] });
})(AddUser);
