...
import { Form, Checkbox, Input, Button, Icon, Badge, Tabs, Modal, Tooltip, Tree, message } from 'antd';
 ...
 * 描述: 用户管理界面
 */
function ChangeUser({ dispatch, pagination, form, loading, changeUserModalState, roles, currentEditUser, selectedOrganizations }) {
    function handleCancel() {
        dispatch({
            type: 'user/setState',
            payload: {
                changeUserModalState: Object.assign({}, changeUserModalState, { visible: false })
            }
        });
    }
    ...
    function onTabChange(value) {
        dispatch({
            type: 'user/setState',
            payload: { changeUserModalState: Object.assign({}, changeUserModalState, { activeTabKey: value }) }
        });
    }
    ...
    // 	return count;
    // }
    return (<Modal visible={changeUserModalState.visible} width={658} title={'修改用户 - ' + (currentEditUser.user ? currentEditUser.user.name : '')} onCancel={handleCancel} footer={[
        <Button key="back" type="primary" loading={loading} htmlType="submit" onClick={() => {
        ...
        initialValue: currentEditUser.user ? currentEditUser.user.isLockoutEnabled : true
				...
				<TabPane tab={<div className={styles.mm}>
							<span>角色</span>
							<Badge count={currentEditUser.roleCount} style={{ backgroundColor: '#5D7DC3', marginLeft: 4 }}/>
						</div>} key="2">
					<Form>
						{currentEditUser.roles ? (currentEditUser.roles.map((ele, index) => {
        return (<FormItem key={index} {...formCol3}>
...
ChangeUser = connect((state) => {
    return Object.assign({}, state.user, { loading: state.loading.effects['user/GetUserPermissionsForEdit'] });
})(ChangeUser);
