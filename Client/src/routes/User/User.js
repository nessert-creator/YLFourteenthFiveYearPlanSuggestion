 ...
 * 描述: 用户管理界面
 */
function User({ dispatch, expand, form, loading, pagination, addUserModalState, changeUserModalState, changePasswordModalState, items, visibleColumnTitles, visibleColumnWidth, filter, selectedUserIds, selectedUsers }) {
    const { getFieldDecorator } = form;
    let form1 = (<Row type="flex" align="middle" className={styles.row1}>
			<Col span={10}>
				<Search placeholder="输入关键字搜索，将搜索[用户名][姓名][邮箱][手机号]" onSearch={filterUser}/>
			</Col>
			<Col span={6} offset={8} className={styles.tR}>
				<a className={styles.collapse} onClick={() => {
        dispatch({
            type: 'user/setState',
            payload: {
                expand: !expand,
    ...
    function searchUser() {
        form.validateFields((err, values) => {
            ...
            if (!err) {
                ...
                dispatch({
                    type: 'user/getUsers',
                    payload: Object.assign({}, data, { current: 1, pageSize: 10 })
                });
            }
        });
    }
    ...
    let form2 = (<div className={styles.div2}>
            ...
            return (<Col span={8} key={index}>
				...
				<Row gutter={24} type="flex" align="middle">
					...
					<Col span={6} className={styles.tR}>
						<a className={`${styles.collapse} ${styles.collapsePR}`} onClick={() => {
        dispatch({
            type: 'user/setState',
            payload: {
                expand: !expand,
            ...
            render: (text, record) => (<div>
                ...
                const menu = (<PermissionWrapper>
                    ...
                    dispatch({
                        ...
                        payload: {
                            id: record.id
									...
									{record.isLocked ? (<Menu.Item requiredPermission="Pages.Administration.Users.Unlock">
											<a onClick={() => {
                    dispatch({
                    ...
                    dispatch({
                        type: 'user/setState',
                        payload: {
                            changePasswordModalState: Object.assign({}, changePasswordModalState, { visible: true, user: Object.assign({}, record) })
                        }
                    });
                }}>
												修改密码
                ...
                return (<PermissionWrapper requiredAnyPermissions="Pages.Administration.Users.Edit,Pages.Administration.Users.Delete">
    ...
    function _onChange(pagination, filters, sorter) {
        dispatch({
            type: 'user/getUsers',
            payload: Object.assign({}, pagination, filter, { sorting: sorter.field ? sorter.field + ' ' + sorter.order.replace('end', '') : '' })
        });
    }
    const rowSelection = {
    ...
    const dropdownMenu = (<PermissionWrapper>
			<Menu>
    ...
    return (<div>
        ...
        dispatch({
            ...
            payload: {
				...
				<Table columns={columns.filter((c) => visibleColumnTitles.indexOf(c.title) >= 0)} dataSource={items} pagination={pagination} loading={loading} onChange={_onChange} rowSelection={rowSelection} rowKey={(record) => (record.id ? record.id : record.key)} bordered={true} size="middle" scroll={{ x: visibleColumnWidth }}/>
			</div>
...
User = connect((state) => {
    return Object.assign({}, state.user, { loading: state.loading.effects['user/getUsers'] });
})(User);
