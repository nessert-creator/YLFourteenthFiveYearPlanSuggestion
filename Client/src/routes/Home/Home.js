...
import { Layout, Menu, Icon, Select, Badge, Avatar, Dropdown, Modal, Button, Input, Upload, Form, Popover, Switch, Checkbox, Spin, Tooltip, Row, Col } from 'antd';
...
class Home extends React.Component {
    ...
    render() {
        const { dispatch, title, menus, mintalk, result, collapsed, loading, visible, children, touxiang, changePassword, changeInformation, openChat, havemessage, openSearchFriend, form, skins, notification, visibleNotification, visibleNotificationPopover, notificationSettings, friends, chatLoading, shouldChangePasswordOnNextLogin, thirdPartyVisible, messageor } = this.props;
        // less.watch();
        ...
        function recursion(dataSource) {
            ...
            return dataSource.map((menu, index) => {
                if (menu.items && menu.items.length) {
                    return (<SubMenu key={menu.name} title={<span>
        ...
        const skinPopover = (<div className={styles.skinListContainer}>
				<Row>
					{skins.map((n, i) => (<Col span={4} key={i}>
							<a style={{ color: n.value, marginRigt: '10px' }} onClick={() => changeSkinHandle(n.value)} key={i}>
								<span>■</span>
								<span>{n.name}</span>
							</a>
						</Col>))}
        ...
        const notificationsList = (<div>
				<div>
					{notification.items.map((n, i) => (<div key={n.id} className={styles.notificationsContent}>
        ...
        return (<Layout style={{ height: '100%' }}>
				<Sider trigger={null} collapsible collapsed={collapsed} onCollapse={(collapsed, type) => {
            //展开-收起时的回调函数，有点击 trigger 以及响应式反馈两种方式可以触发
            toggle();
            // console.log(collapsed);
            // console.log(type);
        }} breakpoint="lg" style={{ background: '#fff' }}>
				...
				<Layout>
					<Header className={styles.headerD}>
						...
						<div className={styles.headbox}>
							...
							<Popover visible={visibleNotificationPopover} onVisibleChange={handleVisibleNotificationPopoverChange} content={notificationsList} title={<span>
            ...
            dispatch({
                ...
                payload: {
                    ...
                    visibleNotificationPopover: false
							...
							<div className={styles.headpor}>
								<Avatar src={result.user ? `${remoteUrl}` + result.user.profile : ''} size="small" className={styles.head}/>
							...
							<a style={{ lineHeight: 1 }}>
								{chatLoading ? (<Tooltip placement="bottomRight" title={'正在连接聊天服务器'}>
										<Spin />
									</Tooltip>) : (<div>
										<Badge count={havemessage && messageor ? messageor.side == 2 ? 'new' : '' : ''}>
											<img onClick={showchat} style={{ width: '26px', height: '26px' }} src={chatImage}/>
										</Badge>
										{openChat ? <Chat /> : null}
				...
				{changePassword || shouldChangePasswordOnNextLogin ? (<Modal visible={changePassword || shouldChangePasswordOnNextLogin} title={shouldChangePasswordOnNextLogin ? '第一次登录需要修改密码' : '修改密码'} closable={!shouldChangePasswordOnNextLogin} onOk={handleOk} onCancel={handleCancel} width="450px" footer={[
            shouldChangePasswordOnNextLogin ? null : (<Button key="back" onClick={handleCancel}>
									取消
								</Button>),
            ...
            rules: [
				...
				{changeInformation ? (<Modal visible={changeInformation} title="修改个人信息" width="550" onCancel={handleCancel3} footer={[
            ...
            <Button key="submit" type="primary" loading={loading} onClick={handleOkConfirm}>
						...
						<div style={{ height: 200 }}>
							<div className={styles.upface}>
								...
								<Upload {...props}>
									<Button type="ghost" className={styles.upbox} style={{
            backgroundSize: 'contain',
            backgroundImage: 'url(' + (result.user ? `${remoteUrl}` + result.user.profile : '') + ')'
        }}>
							...
							<div className={styles.upshow}>
								...
								<Form>
									<FormItem label="姓名：" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} style={{ marginTop: 20 }}>
            ...
            initialValue: result.user ? result.user.phoneNumber : ''
				...
				<Modal visible={visibleNotification} title="通知设置" onOk={handleOkNotification} onCancel={handleCancelNotification} width="450px" footer={[
            ...
            <Button key="submit" type="primary" loading={loading} onClick={handleOkNotification}>
					...
					<Form>
						...
						<FormItem help="这个选项可以用来完全启用/禁用接收通知。" labelCol={{ span: 6 }} wrapperCol={{ span: 15 }}>
							<Switch checked={notificationSettings.receiveNotifications} onChange={(chenked) => {
            dispatch({
                ...
                payload: {
                    receiveNotifications: chenked
						...
						<h2>通知类型</h2>
						{notificationSettings.receiveNotifications ? ('') : (<div style={{ color: '#ed6b75' }}>
								<br />您完全禁用接收通知,您可以启用它并选择您想收到通知类型。<br />
								<br />
							</div>)}
						{notificationSettings.notifications.map((n, i) => (<div key={n.name}>
            ...
            valuePropName: n.isSubscribed ? 'checked' : 'unchecked'
				...
				<UserLogin />
				<div className={styles.mintalkbox} style={mintalk ? { display: 'none' } : { display: 'block' }} onClick={mintalkclose}>
					<div className={havemessage && messageor ? messageor.side == 2 ? (styles.mintalkcallget) : (styles.mintalkcall) : (styles.mintalkcall)}>
...
Home = connect((state) => {
    return Object.assign({}, state.home, { thirdPartyVisible: state.indexpage.setting.alipayIsEnabled ||
