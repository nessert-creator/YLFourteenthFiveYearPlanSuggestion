...
import { Button, Form, Input, Icon, Tabs, Popover, Badge, Tooltip } from 'antd';
...
function Chat({ dispatch, openChat, form, showtexts, morenshow, friends, searchfriends, openSearchFriend, friendPagination, sendvalue, mybbtodo, allmessage, result, display, shownew }) {
    ...
    return (<div className={styles.talkshowmodal} style={display ? { width: 1000 } : { width: 400 }}>
				...
				<div className={styles.listbox}>
					<div className={styles.friendlist} style={display ? { width: '40%' } : { width: '400px' }}>
						<div className={styles.searchfriend}>
							<Search placeholder="搜索好友" style={{ width: '60%', margin: '10px 20px', fontSize: '20px' }} onSearch={(value) => {
        dispatch({
            type: 'home/setState',
            payload: {
						...
						<div className={styles.toalllist}>
							<Tabs defaultActiveKey="1q">
								<TabPane tab="好友" key="1q">
									{friends.friends ? (friends.friends.map((value, index) => {
        return (<p className={styles.everylist} key={index} onClick={() => {
            totalk(value);
        }}>
													<Badge count={value.badge ? value.badge : 0} style={{ position: 'absolute', left: -400, top: -20 }}/>
													<div className={styles.touxiang} style={value.isOnline ? ({}) : ({
                ...
                : '')} className={styles.touimg}/>
													...
													<div className={styles.listalltime}>
														<div className={styles.listtime}>
															{value.isOnline ? (<span style={{ color: 'red' }}>在线</span>) : ('离线')}
														</div>
														<div className={styles.listtime2}>
															<span style={{ color: 'red' }}>
																{display ? ('') : value.unreadMessageCount !== 0 ? (value.unreadMessageCount + '未读') : ('')}
															</span>
														</div>
													</div>
												</p>);
								...
								<TabPane tab="黑名单" key="2q">
									{friends.blockes ? (friends.blockes.map((value, index) => {
        return (<p className={styles.everylist} key={index} onClick={() => {
            totalk(value);
        }}>
													<div className={styles.touxiang} style={value.isOnline ? ({}) : ({
					...
					<div className={styles.bblist} style={display ? { display: 'block' } : { display: 'none' }}>
						<div className={styles.startbb}>
							<div className={styles.bbname}>
								...
								{sendvalue.isOnline ? (<span style={{ color: 'red', marginLeft: 20 }}>· 在线</span>) : (<span style={{ color: '#ccc', marginLeft: 20 }}>· 离线</span>)}
							</div>
						...
						<div className={styles.liaotian} ref={(node) => {
        refDom = node;
        refDom ? (refDom.scrollTop = refDom.scrollHeight) : '';
    }}>
							{allmessage && allmessage.items ? (allmessage.items.map((value, index) => (<div className={styles.bigobox}>
										<div className={styles.showtalk}><img className={styles.imgbox} src={remoteUrl +
        (value.side == 1
            ? result.user.profile
            : '/Profile/GetProfilePictureById/' +
                (sendvalue.friendProfilePictureId || ''))} style={value.side == 1 ? { float: 'right' } : { float: 'left' }}/>
										<div className={styles.talkbox} style={value.side == 1 ? { float: 'right' } : { float: 'left' }}>
											{value.message}
										</div></div>
										<br />
										<div className={styles.timebox}>{formatMsgTime(value.creationTime)}</div>
									</div>))) : ('')}
							{mybbtodo ? (mybbtodo.map((value, index) => (value.targetUserId == sendvalue.friendUserId ?
        <div className={styles.bigobox}>
										<div className={styles.showtalk}><img className={styles.imgbox} src={remoteUrl +
            (value.side == 1
                ? result.user.profile
                : '/Profile/GetProfilePictureById/')} style={value.side == 1 ? { float: 'right' } : { float: 'left' }}/>
										<div className={styles.talkbox} style={value.side == 1 ? { float: 'right' } : { float: 'left' }}>
											{value.message}
										</div></div>
										<br />
						...
						<div className={styles.sendbb}>
							...
							<div className={styles.writesend}>
								<FormItem>
									{getFieldDecorator('textshow', {})(<TextArea rows={3} style={{
        resize: 'none',
        border: 'none',
        outline: 'none',
        fontSize: '18px',
        ...
        boxShadow: 'none'
								...
								<Button type="primary" style={{ fontWeight: '900', float: 'right', marginRight: '20px' }} onClick={sendme}>
									发送
								</Button>
								<Button type="primary" style={{ fontWeight: '900', float: 'right', marginRight: '20px' }} onClick={closebox}>
									关闭
								</Button>
							</div>
						</div>
