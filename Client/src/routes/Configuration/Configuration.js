...
import { Form, Button, Row, Col, Icon, Tabs, Upload, Modal, Checkbox, Radio, InputNumber } from 'antd';
...
function Configuration({ dispatch, allSetting, form, mode, passwordComplexitySetting }) {
    ...
    return (<div>
        ...
        form.validateFields((err, values) => {
			...
			<Tabs>
				{allSetting.map((setting) => (<TabPane tab={setting.tabName} key={setting.name} forceRender={true}>
						{setting.name == 'SystemSettingDto' ? (<div>
								<h2>应用LOGO</h2>
								<br />
								<Upload {...props}>
									<Button type="ghost">
								...
								<div style={{ width: '300px' }}>
									{getFieldDecorator(setting.name)(<Setting data={setting.setting.filter((s) => s.name != 'SystemSettingDto')}/>)}
								</div>
							</div>) : setting.name == 'SecuritySettingDto' ? (<div>
        ...
        initialValue: passwordComplexitySetting.MaxLength
								...
								<div style={{ width: '300px' }}>
									{getFieldDecorator(setting.name)(<Setting data={setting.setting.filter((s) => s.name != 'PasswordComplexity')}/>)}
								</div>
