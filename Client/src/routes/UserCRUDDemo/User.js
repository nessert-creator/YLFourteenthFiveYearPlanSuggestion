...
import { Form, Input, Badge, Menu, Modal, Dropdown, Icon, Button } from 'antd';
...
function User({ form, record, addUserModalState, changeUserModalState, changePasswordModalState, selectedUserIds, selectedUsers }) {
    const columns = [
        {
            title: '用户名',
            dataIndex: 'userName',
    ...
    return (<div>
			<CRUD form={form} getAllApi={new api.UserApi().appUserGetUsers} toExcelApi={new api.UserApi().appUserGetUsersToExcel} columns={columns} filterProps={{
        filters: [
            {
    ...
    }} tableProps={{ rowSelection: rowSelection }} perToolButtons={<PermissionWrapper requiredPermission="Pages.Administration.Users.Create">
						<Button style={{ marginRight: '12px' }} type="primary" onClick={() => {
        dispatch({
            type: 'user/setState',
            payload: {
                ...
                selectedOrganizations: []
					...
					</PermissionWrapper>} postToolButtons={<Dropdown overlay={dropdownMenu}>
						<Button>
							更多操作<Icon type="down"/>
						</Button>
					</Dropdown>}/>
...
User = connect((state) => {
    return Object.assign({}, state.user, state.crud);
})(User);
