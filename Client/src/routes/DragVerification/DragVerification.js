...
class DragVerification extends React.Component {
    constructor() {
        ...
        this.MouseDownHandle = (e) => {
            this.setState(Object.assign({}, this.state, { isMove: true, x: 0, startX: -1, arrayDate: [], startDate: new Date() }));
        ...
        this.MouseMoveHandle = (e) => {
            if (this.state.startX < 0) {
                ...
                if (tempX >= 0 && tempX <= this.props.imgX - 40) {
                    this.setState(Object.assign({}, this.state, { x: tempX, arrayDate: [...this.state.arrayDate, [tempX, new Date().getTime()]] }));
                }
            }
        };
        this.MouseUpHandle = (e) => {
            ...
            this.setState(Object.assign({}, this.state, { isMove: false, x: 0, startX: -1 }));
            this.props.dispatch({
    ...
    render() {
        ...
        return (<div className={styles.content}>
            ...
            //当前x轴偏移量
            x = x + (num - i) * -cutX;
            return <div key={i} style={Object.assign({}, cutStyle, { backgroundPosition: x + 'px ' + y + 'px' })}/>;
        })}
				</div>
				<div className={styles.dragImg} style={{
            backgroundImage: 'url(' + small + ')',
            top: y,
            left: success ? myX : this.state.x
        }}/>
				<div className={styles.bord}>
					<div className={styles.dragBack} style={{
            width: success ? '100%' : this.state.x
        }}/>
					<div className={styles.dragText}>{success ? '验证通过' : '拖动图片验证'}</div>
					{success ? null : (<div className={styles.handler} onMouseDown={this.MouseDownHandle} style={{
            left: this.state.x
        }}>
							<Icon type="double-right"/>
						</div>)}
					{success ? (<div className={styles.handler} style={{
            right: 0,
            left: 'initial'
        }}>
							<Icon type="like"/>
						</div>) : (<a className={styles.refesh} title="点击刷新验证码" onClick={this.refeshHandle}>
							<Icon type="sync"/>
						</a>)}
				</div>
			</div>);
...
export default connect((state) => {
    return Object.assign({}, state.dragVerification);
})(DragVerification);
