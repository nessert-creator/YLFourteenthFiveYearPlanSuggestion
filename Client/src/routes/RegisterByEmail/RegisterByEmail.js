...
function RegisterByEmail({ dispatch, form, captcha, abled, hedhtml, thirdPartyToken }) {
        ...
        form.validateFields(['emailAddress', 'captcha'], (err, values) => {
            if (!err) {
                ...
                dispatch({
                    ...
                    payload: Object.assign({}, values, { email: values.emailAddress })
                });
            }
        });
    }
    ...
    return (<div className={styles.navbox}>
    ...
    })(<Input prefix={<Icon type="user" style={{ fontSize: 13 }}/>} placeholder="4-16个字符，允许中文" style={{ marginTop: '5px', height: 40 }}/>)}
					</FormItem>
					<FormItem>
						<span className={styles.colorsize}>邮箱：</span>
						{getFieldDecorator('emailAddress', {
        rules: [{ required: true, message: '请输入邮箱！' }]
    })(<Input prefix={<Icon type="mail" style={{ fontSize: 13 }}/>} placeholder="邮箱" style={{ marginTop: '5px', height: 40 }}/>)}
					</FormItem>
        ...
        rules: [{ required: true, message: '请输入邮箱验证码！' }]
								...
								<Input prefix={<Icon type="mail" style={{ fontSize: 13 }}/>} placeholder="邮箱验证码" style={{ width: '230px', float: 'left', height: 40 }}/>
								<Button type="primary" htmlType="submit" style={{ float: 'left', marginLeft: '10px', width: '113px', height: 40 }} onClick={getcaptch} disabled={abled}>
									{hedhtml}
								</Button>
							</div>)}
					</FormItem>
    ...
    })(<Input prefix={<Icon type="lock" style={{ fontSize: 13 }}/>} type="password" placeholder="密码长度6-20位" style={{ marginTop: '5px', height: 40 }}/>)}
					</FormItem>
					<FormItem>
...
RegisterByEmail = connect((state) => {
    return Object.assign({}, state.registerByEmail, { thirdPartyToken: state.indexpage.thirdPartyToken });
})(RegisterByEmail);
