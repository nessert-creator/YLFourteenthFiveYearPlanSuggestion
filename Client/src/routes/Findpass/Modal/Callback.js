...
import { Form, Icon, Input, Button, Steps, notification } from 'antd';
...
function Callback({ dispatch, form, hedhtml, abled, loading }) {
    ...
    return (<div>
      ...
      <Form onSubmit={getcaptch} className={`${styles.formbox} login-form`}>
    ...
    })(<Input prefix={<Icon type="phone" style={{ fontSize: 13 }}/>} placeholder="手机号" style={{ marginTop: '5px', height: 40 }}/>)}
        </FormItem>
        <FormItem>
        ...
        rules: [{ required: true, message: '请输入短信验证码！' }],
              ...
              <Input prefix={<Icon type="shake" style={{ fontSize: 13 }}/>} placeholder="短信验证码" style={{ width: '230px', float: 'left', height: 40 }}/>
              <Button type="primary" htmlType="submit" style={{ float: 'left', marginLeft: '10px', width: '113px', height: 40 }} onClick={getcaptch} disabled={abled}>{hedhtml}</Button>
            </div>)}
    ...
    })(<Input prefix={<Icon type="user" style={{ fontSize: 13 }}/>} placeholder="请输入新密码" style={{ marginTop: '5px', height: 40 }} type="password"/>)}
        </FormItem>
        <FormItem>
          <div className={styles.colorsize}>确认新密码：</div>
          {getFieldDecorator('password1', {
        rules: [{ required: true, message: '请再次确认您的密码' }],
    })(<Input prefix={<Icon type="user" style={{ fontSize: 13 }}/>} placeholder="请再次输入新密码" style={{ marginTop: '5px', height: 40 }} type="password"/>)}
        </FormItem>
...
Callback = connect((state) => {
    return Object.assign({}, state.callback, { loading: state.loading.effects['callback/setlogin3'] });
})(Callback);
