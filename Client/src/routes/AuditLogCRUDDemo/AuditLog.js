...
import { Form, Input, Row, Col, Badge, Modal, Button } from 'antd';
...
class AuditLog extends React.Component {
    ...
    render() {
        ...
        return (<div>
				<CRUD form={form} modalWidth={800} getAllApi={new api.AuditLogApi().appAuditLogGetAuditLogs} toExcelApi={new api.AuditLogApi().appAuditLogGetAuditLogsToExcel} columns={columns} filterProps={{
            filters: [
                {
                    name: 'startDate',
                    displayName: '开始时间',
            ...
            advancedFilters: [
				...
				<Modal visible={visible} width={900} title="系统日志详情" onCancel={handleCancel} footer={[
					...
					<div>
						<Row type="flex" gutter={12}>
							<Col span={12} className={styles.row}>
								...
								<h2>自定义数据</h2>
								<Row gutter={24} type="flex">
									{record.customData == null ? (<Col span={24}>没有自定义数据</Col>) : (<Col span={24}>
							...
							<Col span={24} className={styles.row}>
								<h2>错误状态</h2>
								<Row gutter={24} type="flex">
									{record.exception == null ? (<Col span={24}>成功</Col>) : (<Col span={24}>
