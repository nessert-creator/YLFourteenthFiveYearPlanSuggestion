...
function NotificationInfo({ dispatch, expand, form, loading, visible, record }) {
    ...
    return (<Modal visible={visible} width={900} title="通知详情" onCancel={handleCancel} footer={[
				...
				<Row type="flex" gutter={12}>
					<Col span={24} className={styles.row3}>
						...
						<Row gutter={24} type="flex">
							<Col span={4}>标题:</Col>
							<Col span={20}>
								{record.notification.data.properties.message ? (record.notification.data.properties.message) : ('系统通知')}
							</Col>
						</Row>
						<h2>通知详情</h2>
						<Row gutter={24} type="flex">
							<Col span={24}>
								{record.notification.data.properties.content ? (record.notification.data.properties.content) : (record.notification.data.properties.message)}
							</Col>
						</Row>
					</Col>
				</Row>
