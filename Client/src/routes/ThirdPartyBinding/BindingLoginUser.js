    ...
    componentDidMount() {
        ...
        if (!match.test(paramString)) {
            this.props.dispatch({
                ...
                payload: {
                    bindingResult: Object.assign({}, this.props.bindingResult, { isBinding: false, message: '未能获取第三方授权码，绑定失败' })
                }
            });
            return;
        }
        ...
        if (!routerMatch.test(paramString)) {
            this.props.dispatch({
                type: 'thirdpartybinding/setState',
                payload: {
                    bindingResult: Object.assign({}, this.props.bindingResult, { isBinding: false, message: '未能获取第三方平台信息，绑定失败' })
                }
            });
            return;
        }
    ...
    render() {
        ...
        return (<div className={styles.navbox}>
				...
				<div className={styles.indexbox}>
					...
					{!bindingResult.isBinding & bindingResult.success ? (<div>
							<Row>
								<Col span={3}>
