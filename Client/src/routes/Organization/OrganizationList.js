...
import { Modal, Tree, Button, Icon, Row, Col, Dropdown, Menu, Table } from 'antd';
 ...
 * 描述: 组织机构管理界面
 */
function OrganizationList({ dispatch, expand, organizations, editOrganizationModalVisible, userList, userPagination, userLoading, organization, addOrganizationUserModalVisible }) {
    ...
    function findNode(root, id) {
        ...
        for (var i in root) {
            if (root[i].id == id) {
                const node = Object.assign({ parent: 0 }, root[i]);
                return node;
            }
    ...
    const loopOrganizations = (data) => data.map((item) => {
				...
				<Col span={11} className={styles.normalC}>
					<Row className={styles.titlehead}>
						...
						<Col span={12} className={styles.noradd}>
							<PermissionWrapper requiredPermission="Pages.Administration.OrganizationUnits.ManageOrganizationTree">
								<Button type="primary" className={styles.norbut} onClick={() => {
        dispatch({
            type: 'organization/setState',
            payload: {
                isAddOrganization: true,
				...
				<Col span={11} className={styles.normalD}>
					<Row className={styles.titlehead}>
						...
						<Col span={12} className={styles.noradd}>
							<PermissionWrapper requiredPermission="Pages.Administration.OrganizationUnits.ManageMembers">
								<Button type="primary" disabled={!organization || !organization.id} onClick={() => {
        dispatch({
        ...
        dispatch({
            ...
            payload: {
                addOrganizationUserModalVisible: !addOrganizationUserModalVisible
					...
					<Row>
						<Col>
							<Table columns={columns} pagination={userPagination} loading={userLoading} rowKey={(record) => record.id} dataSource={userList.items} onChange={handleTableChange}/>
						</Col>
					</Row>
				</Col>
			</Row>
