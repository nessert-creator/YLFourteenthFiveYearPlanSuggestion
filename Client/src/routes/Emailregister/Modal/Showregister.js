...
function Showregister({ dispatch, form, thirdPartyToken }) {
    ...
    })(<Input prefix={<Icon type="user" style={{ fontSize: 13 }}/>} placeholder="1-32个英文字母或数字" style={{ marginTop: '5px', height: 40 }}/>)}
        </FormItem>
        <FormItem>
    ...
    })(<Input prefix={<Icon type="mail" style={{ fontSize: 13 }}/>} placeholder="请输入邮箱地址" style={{ marginTop: '5px', height: 40 }}/>)}
        </FormItem>
        <FormItem>
          <span className={styles.colorsize}>密码：</span>
          {getFieldDecorator('password', {
        rules: [{ required: true, message: '请输入密码！' }],
    })(<Input prefix={<Icon type="lock" style={{ fontSize: 13 }}/>} type="password" placeholder="密码长度8-20位" style={{ marginTop: '5px', height: 40 }}/>)}
        </FormItem>
        <FormItem>
...
Showregister = connect((state) => {
    return Object.assign({}, state.Showregister, { thirdPartyToken: state.indexpage.thirdPartyToken });
})(Showregister);
