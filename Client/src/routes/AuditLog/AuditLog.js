...
import { Form, Table, Input, Button, Row, Col, Icon, DatePicker, Select, InputNumber, Badge } from 'antd';
...
function AuditLog({ dispatch, expand, form, loading, pagination, visible, items }) {
    ...
    let from1 = (<div>
        ...
        initialValue: [moment().subtract(3, 'days'), moment()]
					...
					<PermissionWrapper>
						<Button type="primary" permisions={''} visible={false} onClick={() => {
        form.validateFields((err, values) => {
            if (!err) {
                var payload = Object.assign({}, pagination, values, { isAdvancedSearch: false });
                dispatch({
                    type: 'auditLog/getAuditLogs',
                    payload: payload
                });
					...
					<a className={styles.collapseRight} onClick={() => {
        console.log(expand);
        dispatch({
            type: 'auditLog/setState',
            payload: {
    ...
    let from2 = (<div className={styles.form2}>
        ...
        form.validateFields((err, values) => {
            if (!err) {
                var payload = Object.assign({}, pagination, values, { isAdvancedSearch: true });
                dispatch({
                    type: 'auditLog/getAuditLogs',
                    payload: payload
                });
        ...
        form.resetFields();
			...
			<a className={styles.collapseRight} style={{ bottom: 22 }} onClick={() => {
        console.log(expand);
        form.resetFields();
        dispatch({
            type: 'auditLog/setState',
    ...
    return (<div>
        ...
        form.validateFields((err, values) => {
            if (!err) {
                var payload = Object.assign({}, pagination, values, { isAdvancedSearch: visible });
                dispatch({
...
AuditLog = connect((state) => {
    return Object.assign({}, state.auditLog, { loading: state.loading.effects['auditLog/getAuditLog'] });
})(AuditLog);
