...
import { Table, Button, Input, Modal, Tabs, Tree, Tag, Row, Col } from 'antd';
...
function Role({ dispatch, roles, addRoleModalState, editRoleModalState, editingRole, permissionTree }) {
    ...
    return (<div>
			<Row type="flex" align="middle" className={styles.row1}>
				<Col span={8}>
					<Search placeholder="输入关键字搜索" onSearch={(value) => dispatch({
        type: 'role/getRoles',
        payload: {
            filter: value
        }
			...
			<div className={styles.tabbox}>
				...
				<Table dataSource={roles} bordered={true} size="middle" rowKey={(record) => record.id} pagination={false}>
					<Column title="角色名称" dataIndex="displayName" key="displayName"/>
					<Column title="角色标签" key="label" render={(text, record) => {
        ...
        return <span />;
					...
					<Column title="操作" key="action" render={(text, record) => (<PermissionWrapper requiredAnyPermissions="Pages.Administration.Roles.Edit,Pages.Administration.Roles.Delete">
								<span>
