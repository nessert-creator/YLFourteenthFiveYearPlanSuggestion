...
import { Table, Button, Input, Modal, Row, Col, Badge } from 'antd';
...
function AppEdition({ dispatch, items, loading, pagination, modalVisible }) {
    ...
    function _onChange(pagination, filters, sorter) {
        dispatch({
            ...
            payload: Object.assign({}, pagination, sorter)
        });
    }
    return (<div>
			<Row type="flex" align="middle" className={styles.row1}>
				<Col span={8}>
					<Search placeholder="输入版本号搜索" size="large" onSearch={(value) => dispatch({
        type: 'appEdition/getAppEditions',
        payload: {
            filter: value
        }
			...
			<div className={styles.tabbox}>
				...
				<Table dataSource={items} bordered={true} size="middle" rowKey={(record) => record.id} pagination={pagination} loading={loading} onChange={_onChange}>
					<Column title="系统" dataIndex="appType" sorter/>
					<Column title="版本号" dataIndex="version" sorter/>
					<Column title="关于" dataIndex="aboutUrl" render={(text, record) => {
        return (<a target="_blank" href={record.aboutUrl}>
									关于
								</a>);
    }}/>
					<Column title="是否强制更新" sorter dataIndex="isMandatoryUpdate" render={(text, record) => {
        return (record.isMandatoryUpdate ?
            <Badge status="success" text="是"/>
            :
                <Badge status="default" text="否"/>);
    }}/>
					<Column title="是否启用" sorter dataIndex="isActive" render={(text, record) => {
        return (record.isActive ?
            <Badge status="success" text="是"/> : <Badge status="default" text="否"/>);
    }}/>
					<Column title="描述" dataIndex="describe" render={(text, record) => {
        return <div className={styles.tableText}>{record.describe}</div>;
    }}/>
					<Column title="Itunes连接" dataIndex="itunesUrl" render={(text, record) => {
        return record.appType == 'IOS' ? (<a target="_blank" href={record.itunesUrl} disabled={!Boolean(record.itunesUrl)}>
									Itunes连接
								</a>) : null;
    }}/>
					<Column title="创建时间" dataIndex="creationTime" sorter/>
					<Column title="操作" key="action" render={(text, record) => (<span>
								<a disabled={!Boolean(record.installationPackage)} target="_blank" href={remoteUrl + '/api/services/app/appEditions/DownloadAppEdition?id=' + record.id}>
									下载
								</a>
...
AppEdition = connect((state) => {
    return Object.assign({}, state.appEdition, { loading: state.loading.effects['appEdition/getAppEditions'] });
})(AppEdition);
