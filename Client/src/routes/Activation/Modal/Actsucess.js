...
function Actsucess({ dispatch, form }) {
    ...
    return (<div>
      <Form onSubmit={handleSubmit} className={`${styles.formbox} login-form`}>
        <div style={{ marginTop: 20, height: '80px', lineHeight: '80px' }}><span style={{ fontSize: '50px', color: 'red', float: 'left', fontWeight: '900' }}><Icon type="check-circle-o"/></span><span style={{ fontSize: '30px', color: '#000', marginLeft: 20, float: 'left', fontWeight: '900' }}>发送成功</span></div>
...
Actsucess = connect((state) => {
    return Object.assign({}, state.Actsucess);
})(Actsucess);
