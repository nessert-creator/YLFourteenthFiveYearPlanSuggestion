    ...
    render() {
        ...
        return (<div className={styles.normal}>
				<header className={styles.headerbox}>
					...
					{this.props.goToCode == 2 ? (<Link to="/" className={styles.headback}>
							返回登录
						</Link>) : null}
            ...
            rules: [{ required: true, message: '请输入邮件验证码！' }]
							...
							<Col span={10}>
								<Button type="primary" disabled={this.state.btn1} className={styles.smsBtn} onClick={this.sendEmail}>
									{this.state.btnname}
								</Button>
							</Col>
						</Row>
...
Email = connect((state) => {
    return Object.assign({}, state.Sendemail, { loading: state.loading.effects['sendemail/confirmEmailByCode'] });
})(Email);
