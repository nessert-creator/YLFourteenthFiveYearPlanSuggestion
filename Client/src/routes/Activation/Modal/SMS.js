        ...
        form.validateFields((err, values) => {
            if (!err) {
                ...
                dispatch({
                    ...
                    payload: Object.assign({}, values, { sufn: () => {
    ...
    render() {
        ...
        return (<div className={styles.normal}>
				<header className={styles.headerbox}>
					...
					{this.props.goToCode == 1 || this.props.goToCode == 3 ? (<Link to="/" className={styles.headback}>
							返回登录
						</Link>) : null}
				</header>
            ...
            rules: [{ required: true, message: '请输入手机号！' }]
					...
					<FormItem>
						...
						{this.state.smsRandom ? (<center>
								{getFieldDecorator('captcha', {
            rules: [{ required: true, message: '请点击验证！' }]
        })(<Geetest />)}
							</center>) : (<Button type="primary" disabled={this.state.btn1} className={styles.smsBtn} onClick={() => {
            ...
            rules: [{ required: true, message: '请输入短信验证码！' }]
							...
							<Col span={10}>
								<Button type="primary" disabled={this.state.btn1} className={styles.smsBtn} onClick={this.sendSMS}>
									{this.state.btnname}
								</Button>
							</Col>
						</Row>
					</FormItem>
					<Button type="primary" disabled={this.state.successClass} icon={this.state.successClass ? 'check-circle-o' : ''} htmlType="submit" className={this.state.successClass ? styles.login1 : styles.login} loading={loading}>
...
SMS = connect((state) => {
    return Object.assign({}, state.Sendemail, { loading: state.loading.effects['sendemail/confirmPhoneNumberByCode'] });
})(SMS);
