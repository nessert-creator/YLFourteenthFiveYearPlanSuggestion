...
function Demo({ form, record }) {
    const columns = [
        ...
        },
        {
            ...
            sorter: true,
            render: (text, record) => text ? <Badge status="default" text="错误"/> : <Badge status="success" text="成功"/>
        },
        {
            title: '排序',
            dataIndex: 'sort',
    ...
    return (<CRUD form={form} getAllApi={new api.DemoApi().appDemoGetAll} deleteApi={new api.DemoApi().appDemoDelete} updateApi={new api.DemoApi().appDemoUpdate} createApi={new api.DemoApi().appDemoCreate} deleteBatchApi={new api.DemoApi().appDemoDeleteBatch} createPermission="Pages.DemoMangeCreate" updatePermission="Pages.DemoMangeUpdate" deletePermission="Pages.DemoMangeDelete" columns={columns} formNode={formNode} filterProps={{
        filters,
        searchProvide: 'sql'
    }}/>);
}
