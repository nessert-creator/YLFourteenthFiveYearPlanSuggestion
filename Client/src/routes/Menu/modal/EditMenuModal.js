...
import { Modal, Form, Input, Button, Select, Radio, TreeSelect } from 'antd';
...
function EditMenuModal({ dispatch, form, isEditingMenu, editMenuModalVisible, menu, parentId, permissions }) {
    ...
    const permissionOptions = (data) => data.map((permission) => {
        return (<Option value={permission.name}>
    ...
    return (<Modal visible={editMenuModalVisible} width={658} title={<div style={{ fontSize: 20, color: '#000' }}>{isEditingMenu ? '编辑菜单' : '添加菜单'}</div>} onCancel={handleCancel} footer={[
    ...
    })(<TreeSelect dropdownStyle={{ maxHeight: 400, overflow: 'auto' }} placeholder="选择权限" allowClear treeDefaultExpandAll>
