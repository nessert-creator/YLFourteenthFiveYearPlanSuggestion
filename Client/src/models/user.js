...
export default {
    ...
    reducers: {
        ...
        setPages(state, { payload }) {
            return Object.assign({}, state, { items: payload.items, pagination: Object.assign({}, state.pagination, payload.pagination) });
        },
        setCurrentEditUserState(state, { payload }) {
            return Object.assign({}, state, { currentEditUser: Object.assign({}, state.currentEditUser, payload) });
        }
    },
    effects: {
        *getUsers({ payload }, { call, put }) {
            const { success, result } = yield call(...createApiAuthParam({
                ...
                payload: Object.assign({}, payload, { maxResultCount: payload.pageSize ? payload.pageSize : 10, skipCount: payload.pageSize * (payload.current - 1) //跳过多少条
                 })
            }));
            if (success) {
                yield put({
        ...
        *batchUnlockUser({ payload }, { call, put }) {
            yield put({
                type: 'batchOperate',
                payload: Object.assign({}, payload, { operate: '解锁', serverUrl: new api.UserApi().appUserBatchUnlockUser })
            });
        },
        *batchActiveUser({ payload }, { call, put }) {
            yield put({
                type: 'batchOperate',
                payload: Object.assign({}, payload, { operate: '启用', isActive: true, serverUrl: new api.UserApi().appUserBatchActiveUser })
            });
        },
        *batchNotActiveUser({ payload }, { call, put }) {
            yield put({
                type: 'batchOperate',
                payload: Object.assign({}, payload, { operate: '禁用', isActive: false, serverUrl: new api.UserApi().appUserBatchActiveUser })
            });
        },
