...
export default function request(url, options) {
    ...
    //请求头
    opts.headers = Object.assign({ 'Content-Type': 'application/json; charset=UTF-8', 'Authorization': 'Bearer ' + token }, opts.headers);
    //请求方法
    let method = opts.method.toLowerCase();
    // console.log(JSON.stringify(opts.body));
    return fetch(`${remoteUrl}${url}`, Object.assign({}, opts, { body: method == 'get' || method == 'delete' ? null : JSON.stringify(opts.body), withCredentials: true, credentials: "include" }))
        .then(nprogress)
        .then(checkStatus)
        .then(parseJSON);
}
